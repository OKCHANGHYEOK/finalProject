...
// 			contentItems[index].classList.add('selected')
   ...
   const myInfoLoadHandler = function() {
      ...
      tag += '                    <button id="mpmodifyBtn">비밀번호 변경</button>';
      tag += '                </div>';
      tag += '            </form>';
      tag += '        </div>';
      tag += '    </div>';
   ...
   menuItems[0].onclick = myInfo
	...
	tag += '                직접 선택하세요.';
	tag += '            </div>';
	tag += '            <input type="file" name="profile" placeholder="프로필 이미지 선택" style="margin-top: 10px;" required>';
	tag += '        </div>';
	tag += '    </div>';
	tag += '    <div>';
	tag += '        <h2>자기소개</h2>';
	tag += '		<textarea class="introduce" placeholder="자기소개를 작성하세요." required>' + dto.introduce + '</textarea>';
	tag += '    </div>';
	tag += '</form>';
